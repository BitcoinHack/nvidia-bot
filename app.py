...
class MainUI:
    def __init__(self):
        ...
        self.start_button_nvidia = PushButton(
            self.nvidia_inputs_box, command=self.start_nvidia, text="start", grid=[1, 0]
        )
        self.stop_button_nvidia = PushButton(
            self.nvidia_inputs_box,
            command=self.stop_nvidia,
            text="stop",
            enabled=False,
            grid=[2, 0],
        )
        self.nvidia_status = Text(
            self.nvidia_box,
            grid=[0, 1],
            align="left",
            bg="black",
